                ...
                <table id="qcTable" class="w-full text-sm text-left text-slate-600 dark:text-slate-400">
                    <thead class="bg-slate-50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-700 uppercase text-[11px] tracking-wider font-bold">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3 text-center">TARGET COMPLETION (QC)</th>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#qcTable').DataTable({
            ...
            columns: [
                ...
                { 
                    ...
                    searchable: false,
                    render: function(data, type, row) {
                        const qcDateRaw = row.qc_target_date || row.delivery_date;
                        if (!qcDateRaw) {
                            return `<span class="text-xs text-gray-400 font-medium">-</span>`;
                        }

                        const targetDate = new Date(qcDateRaw.split('T')[0] + 'T00:00:00');
                        const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
                        ...
                        let timeBadge = '';
                        if (['WAITING_MGM_CHECK', 'WAITING_APPROVAL', 'FINISHED', 'CLOSED'].includes(row.status)) {
                            timeBadge = `<span class="inline-flex items-center gap-1 px-2 py-0.5 text-[11px] font-bold border bg-blue-100 text-blue-700 border-blue-200 dark:bg-blue-900/40 dark:text-blue-300 dark:border-blue-800">
                                <i class="fa-solid fa-check"></i> QC Target Achieved
                            </span>`;
                        } else if (diffDays < 0) {
                        ...
                        return `<div class="flex flex-col items-center gap-1">
                            <div class="text-[11px] text-gray-700 dark:text-gray-200 font-bold">
                                <i class="fa-solid fa-calendar-check text-gray-400 mr-1"></i> Target QC: ${dateFormatted}
                            </div>
                            ${timeBadge}
                        </div>`;
                    }
        ...
        $('#qcTable').on('draw.dt', function() {
            ...
            if(!hasChecked) {
                $('#selectAllParts').prop('checked', false);
            }
        });
