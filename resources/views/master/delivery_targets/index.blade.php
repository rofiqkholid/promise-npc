...
<div class="bg-white dark:bg-gray-800 shadow border border-gray-200 dark:border-gray-700">
    ...
    <div class="p-6">
        ...
        <div class="mb-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <form action="{{ route('master.delivery-targets.index') }}" method="GET" class="w-full sm:w-auto"
                x-data="{ 
                ...
                <div class="relative w-full sm:w-64">
                    ...
                    <input type="text" name="search" x-model="searchQuery" x-ref="searchInput" placeholder="Search target name..." 
            ...
            <table class="w-full text-sm text-left text-slate-600 dark:text-slate-400">
                ...
                <tbody>
                    ...
                    <tr class="bg-white dark:bg-gray-800 border-b dark:border-gray-700 hover:bg-blue-50/50 dark:hover:bg-gray-700/30 transition group">
                        <td class="px-6 py-4 font-medium text-slate-500 dark:text-slate-400">{{ ($targets->currentPage() - 1) * $targets->perPage() + $loop->iteration }}</td>
                        <td class="px-6 py-4 font-bold text-slate-900 dark:text-white">{{ $target->target_name }}</td>
                        <td class="px-6 py-4">
                            @if($target->is_active)
                                <span class="px-2.5 py-1 border text-xs font-semibold bg-green-100 text-green-800 border-green-200 dark:bg-green-900/40 dark:text-green-300 dark:border-green-800">
                                    Active
                                </span>
                            @else
                                <span class="px-2.5 py-1 border text-xs font-semibold bg-gray-100 text-gray-800 border-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600">
                                    Inactive
                                </span>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end gap-1 opacity-50 group-hover:opacity-100 transition">
                                <a href="{{ route('master.delivery-targets.edit', $target->hashed_id) }}" class="text-blue-600 hover:text-blue-800 hover:bg-blue-50 p-2 transition" title="Edit">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </a>
                                <form action="{{ route('master.delivery-targets.destroy', $target->hashed_id) }}" method="POST" class="inline" onsubmit="confirmAction(event, 'Permanently delete this delivery target?')">
                                    @csrf
                                    @method('DELETE')
